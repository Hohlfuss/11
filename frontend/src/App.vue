...
const backendUrl =
  import.meta.env.VITE_BACKEND_URL || "https://one1-x2mf.onrender.com";
...
onMounted(() => {
  ...
  // ADD THIS: Catch server rejections and pop them up on the screen!
...
const updateLocalState = (serverState: any) => {
  ...
  if (serverState.level > prevLevel) {
    addNotif({
      type: "levelup",
      title: `⬆ Level Up! → ${serverState.level}`,
      sub: "Keep going!",
    });
  }
  ...
  if (typeof incomingTools === "string") {
    try {
      incomingTools = JSON.parse(incomingTools);
    } catch (e) {
      incomingTools = null;
    }
  }
  ...
  if (incomingTools && typeof incomingTools === "object") {
    state.tools = {
      woodcutting: {
        handle:
          incomingTools.woodcutting?.handle ??
          state.tools?.woodcutting?.handle ??
          1,
        metal:
          incomingTools.woodcutting?.metal ??
          state.tools?.woodcutting?.metal ??
          1,
        grip:
          incomingTools.woodcutting?.grip ??
          state.tools?.woodcutting?.grip ??
          1,
        enchantment:
          incomingTools.woodcutting?.enchantment ??
          state.tools?.woodcutting?.enchantment ??
          1,
        critChance:
          incomingTools.woodcutting?.critChance ??
          state.tools?.woodcutting?.critChance ??
          1,
        critDamage:
          incomingTools.woodcutting?.critDamage ??
          state.tools?.woodcutting?.critDamage ??
          1,
        bindings:
          incomingTools.woodcutting?.binding ??
          state.tools?.woodcutting?.bindings ??
          1,
      },
      mining: {
        handle:
          incomingTools.mining?.handle ?? state.tools?.mining?.handle ?? 1,
        metal: incomingTools.mining?.metal ?? state.tools?.mining?.metal ?? 1,
        grip: incomingTools.mining?.grip ?? state.tools?.mining?.grip ?? 1,
        enchantment:
          incomingTools.mining?.enchantment ??
          state.tools?.mining?.enchantment ??
          1,
        critChance:
          incomingTools.mining?.critChance ??
          state.tools?.mining?.critChance ??
          1,
        critDamage:
          incomingTools.mining?.critDamage ??
          state.tools?.mining?.critDamage ??
          1,
        bindings:
          incomingTools.mining?.binding ?? state.tools?.mining?.bindings ?? 1,
      },
      foraging: {
        handle:
          incomingTools.foraging?.handle ?? state.tools?.foraging?.handle ?? 1,
        metal:
          incomingTools.foraging?.metal ?? state.tools?.foraging?.metal ?? 1,
        grip: incomingTools.foraging?.grip ?? state.tools?.foraging?.grip ?? 1,
        enchantment:
          incomingTools.foraging?.enchantment ??
          state.tools?.foraging?.enchantment ??
          1,
        critChance:
          incomingTools.foraging?.critChance ??
          state.tools?.foraging?.critChance ??
          1,
        critDamage:
          incomingTools.foraging?.critDamage ??
          state.tools?.foraging?.critDamage ??
          1,
        bindings:
          incomingTools.foraging?.binding ??
          state.tools?.foraging?.bindings ??
          1,
      },
    };
  }
...
const changeView = (
  newView: "main" | "woodcutting" | "mining" | "crafting" | "foraging",
) => {
  state.view = newView;
};
...
const upgradeTool = (
  ...
  part:
    "handle" | "metal" | "grip" | "enchantment" | "critChance" | "critDamage",
) => {
...
const getUpgradeCost = (level?: number) =>
  Math.floor(15 * Math.pow(1.8, (level || 1) - 1));

const getUpgradeResource = (
  skill: "woodcutting" | "mining" | "foraging",
  part: string,
) => {
  if (skill === "mining") {
...
// --- WORKER HELPERS ---
const workersAvailable = computed(
  () => state.workersTotal - state.workerActions.length,
);
const workersOnNode = (nodeId: string) =>
  state.workerActions.filter((w: any) => w.id === nodeId);
...
<template>
  ...
  >
    ...
    >
      {{ n.title }}
      <p v-if="n.sub" class="text-xs font-normal mt-0.5 opacity-70">
        {{ n.sub }}
      </p>
    </div>
  </TransitionGroup>
  ...
  <div
    class="min-h-screen bg-slate-950 text-slate-200 font-sans p-6 flex flex-col md:flex-row gap-6"
  >
    <div
      v-if="!auth.isAuthenticated"
      class="w-full max-w-md mx-auto mt-20 p-6 bg-slate-900 rounded-xl"
    >
      ...
      <div class="space-y-4">
        <input
          ...
          class="w-full p-2 rounded bg-slate-800 text-white"
        />
        <input
          v-model="auth.passwordInput"
          type="password"
        ...
        <input
          v-if="auth.isRegistering"
          v-model="auth.confirmPasswordInput"
          type="password"
        ...
        <button
          v-if="!auth.isRegistering"
          @click="handleLogin"
          class="w-full bg-blue-600 p-2 rounded text-white font-bold"
        >
          Log In
        </button>
        <button
          ...
          class="w-full bg-green-600 p-2 rounded text-white font-bold"
        >
          Register
        </button>
      </div>
      ...
      <button
        ...
        class="mt-4 text-sm text-slate-400 hover:text-white underline"
      >
        {{
          auth.isRegistering
    ...
    >
      ...
      >
        <div class="flex justify-between items-end">
          <div class="flex items-center gap-3">
            <div
              class="bg-blue-600 text-white w-10 h-10 rounded-full flex items-center justify-center font-bold text-xl shadow-lg border border-blue-400"
            >
              {{ state.level }}
            </div>
            <div>
          ...
          <div
            class="flex items-center gap-2 bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-700"
          >
            <User
              :class="
                state.workersTotal > 0 ? 'text-amber-400' : 'text-slate-600'
              "
              :size="18"
            />
        ...
        <div
          class="w-full bg-slate-950 rounded-full h-2.5 mt-1 border border-slate-900"
        >
          <div
      ...
      <div class="p-6 flex-1 flex flex-col">
        <div
          v-if="state.view === 'main'"
          class="flex-1 flex flex-col justify-center gap-4"
        >
          <h2 class="text-2xl font-bold text-center text-slate-400 mb-4">
            Select an Operation
          </h2>
          ...
          >
            <div
              class="bg-green-900/30 p-4 rounded-lg text-green-500 group-hover:scale-110 transition-transform"
            >
              <Axe :size="32" />
            </div>
            <div class="ml-6 flex-1">
              ...
              <p class="text-slate-400 text-sm">
                Chop trees to gather logs for construction.
              </p>
            </div>
            <ChevronRight
          ...
          >
            <div
              class="bg-orange-900/30 p-4 rounded-lg text-orange-500 group-hover:scale-110 transition-transform"
            >
              <Pickaxe :size="32" />
            </div>
            <div class="ml-6 flex-1">
              ...
              <p class="text-slate-400 text-sm">
                Extract ores from the earth for smelting.
              </p>
            </div>
            <ChevronRight
              class="text-slate-500 group-hover:text-white transition-colors"
          ...
          >
            <div
              class="bg-teal-900/30 p-4 rounded-lg text-teal-400 group-hover:scale-110 transition-transform"
            >
              <Leaf :size="32" />
            </div>
            <div class="ml-6 flex-1">
              ...
              <p class="text-slate-400 text-sm">
                Gather wild fibers, silks, and vines.
              </p>
            </div>
            <ChevronRight
              class="text-slate-500 group-hover:text-white transition-colors"
          ...
          <button
            @click="changeView('crafting')"
            class="group relative flex items-center p-6 bg-slate-800 hover:bg-slate-700 rounded-xl border border-slate-700 hover:border-blue-500/50 transition-all text-left mt-4"
          >
            <div
              class="bg-blue-900/30 p-4 rounded-lg text-blue-500 group-hover:scale-110 transition-transform"
            >
              <Wrench :size="32" />
            </div>
            <div class="ml-6 flex-1">
              ...
              <p class="text-slate-400 text-sm">
                Use resources to improve your tools and yield.
              </p>
            </div>
            <ChevronRight
              class="text-slate-500 group-hover:text-white transition-colors"
          ...
          <div class="grid gap-4">
            ...
            >
              ...
              >
                <div class="flex items-center gap-3">
                  <div
                    class="p-2 rounded-md bg-slate-950 border border-slate-800 text-slate-600"
                  >
                ...
                <span
                  class="text-xs font-semibold text-slate-500 bg-slate-800 border border-slate-700 px-3 py-1 rounded-full"
                >
              ...
              <template v-else>
                <div class="flex justify-between items-center">
                  <div class="flex items-center gap-3">
                    <div
                      :class="[
                        'p-2 rounded-md bg-slate-900 border border-slate-700',
                        node.color,
                      ]"
                    >
                      <component :is="node.icon" :size="24" />
                    ...
                    <div>
                      ...
                      <p class="text-xs text-slate-400">
                        Yields: {{ node.yields }} | XP: {{ node.xpReward }}
                      </p>
                    </div>
                  </div>
                ...
                <div v-if="state.manualAction?.id === node.id" class="mt-1">
                  <p
                    class="text-xs text-blue-400 font-medium mb-1 flex justify-between"
                  >
                    <span>Player Progress</span>
                    <span
                      >{{
                        Math.floor(
                          (state.manualAction.progress / node.time) * 100,
                        )
                      }}%</span
                    >
                  </p>
                  <div
                    class="w-full bg-slate-800 rounded-full h-3 mt-2 overflow-hidden border border-slate-700"
                  >
                    <div
                      class="h-full bg-blue-500 transition-all duration-100 ease-linear"
                      :style="{
                        width: `${Math.min((state.manualAction.progress / node.time) * 100, 100)}%`,
                      }"
                    ></div>
                  </div>
                </div>
                ...
                <div
                  v-for="(w, idx) in workersOnNode(node.id)"
                  :key="idx"
                  class="mt-1"
                >
                  <p
                    class="text-xs text-amber-400 font-medium mb-1 flex justify-between"
                  >
                    <span>Worker #{{ idx + 1 }} Automating</span>
                    <span
                      >{{ Math.floor((w.progress / node.time) * 100) }}%</span
                    >
                  </p>
                  <div
                  ...
                  >
                    <div
                      class="h-full bg-amber-500 transition-all duration-100 ease-linear"
                      :style="{
                        width: `${Math.min((w.progress / node.time) * 100, 100)}%`,
                      }"
                    ></div>
                  </div>
                </div>
        ...
        <div v-if="state.view === 'crafting'" class="flex flex-col h-full">
          ...
          <div class="grid gap-6">
            <!-- HATCHET UPGRADES -->
            <div
              class="bg-slate-800 p-5 rounded-xl border border-slate-700 shadow-sm"
            >
              <h3
                class="text-lg font-bold text-white flex items-center gap-2 mb-4 border-b border-slate-700 pb-2"
              >
              ...
              <div class="flex flex-col gap-4">
                <div
                  class="flex justify-between items-center bg-slate-900 p-3 rounded-lg border border-slate-800"
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Reinforced Handle (Lvl
                      {{ state.tools?.woodcutting?.handle || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Increases chopping speed by 25%
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'handle')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.handle)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('woodcutting', 'handle')
                      ] || 0) < getUpgradeCost(state.tools?.woodcutting?.handle)
                    "
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Sharpened Metal (Lvl
                      {{ state.tools?.woodcutting?.metal || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Increases log yield by +1
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'metal')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.metal)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('woodcutting', 'metal')
                      ] || 0) < getUpgradeCost(state.tools?.woodcutting?.metal)
                    "
                    class="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                    Upgrade
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Firm Grip (Lvl {{ state.tools?.woodcutting?.grip || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      5% chance per level to double yield
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'grip')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.grip)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.woodcutting?.grip) }}
                      {{ getUpgradeResource("woodcutting", "grip") }}
                    </p>
                  </div>
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('woodcutting', 'grip')
                      ] || 0) < getUpgradeCost(state.tools?.woodcutting?.grip)
                    "
                    class="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                    Upgrade
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Nature Enchantment (Lvl
                      {{ state.tools?.woodcutting?.enchantment || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      5% chance per level to double XP
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'enchantment')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.enchantment)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                ...
                >
                  <div>
                    <h4 class="font-bold text-yellow-300/80">
                      ⚡ Lucky Strike Chance (Lvl
                      {{ state.tools?.woodcutting?.critChance || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      +5% crit chance per level — crits multiply yield
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'critChance')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.critChance)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                ...
                >
                  <div>
                    <h4 class="font-bold text-orange-300/80">
                      🔥 Lucky Strike Power (Lvl
                      {{ state.tools?.woodcutting?.critDamage || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Crit yield ×{{
                        (state.tools?.woodcutting?.critDamage || 1) + 1
                      }}
                    ...
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('woodcutting', 'critDamage')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.woodcutting?.critDamage)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
            ...
            >
              ...
              <div class="flex flex-col gap-4">
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Sturdy Handle (Lvl {{ state.tools?.mining?.handle || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                    ...
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'handle')
                        ] || 0) >= getUpgradeCost(state.tools?.mining?.handle)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.mining?.handle) }}
                      {{ getUpgradeResource("mining", "handle") }}
                    </p>
                  </div>
                  <button
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Hardened Head (Lvl {{ state.tools?.mining?.metal || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Increases ore yield by +1
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'metal')
                        ] || 0) >= getUpgradeCost(state.tools?.mining?.metal)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.mining?.metal) }}
                      {{ getUpgradeResource("mining", "metal") }}
                    </p>
                  </div>
                  <button
                    ...
                    :disabled="
                      (state.inventory[getUpgradeResource('mining', 'metal')] ||
                        0) < getUpgradeCost(state.tools?.mining?.metal)
                    "
                    class="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Steady Grip (Lvl {{ state.tools?.mining?.grip || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      5% chance per level to double yield
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'grip')
                        ] || 0) >= getUpgradeCost(state.tools?.mining?.grip)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.mining?.grip) }}
                      {{ getUpgradeResource("mining", "grip") }}
                    </p>
                  </div>
                  <button
                    ...
                    :disabled="
                      (state.inventory[getUpgradeResource('mining', 'grip')] ||
                        0) < getUpgradeCost(state.tools?.mining?.grip)
                    "
                    class="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                ...
                >
                  <div>
                    ...
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'enchantment')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.mining?.enchantment)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('mining', 'enchantment')
                      ] || 0) < getUpgradeCost(state.tools?.mining?.enchantment)
                    "
                    class="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                    Upgrade
                ...
                >
                  <div>
                    <h4 class="font-bold text-yellow-300/80">
                      ⚡ Prospector's Eye (Lvl
                      {{ state.tools?.mining?.critChance || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      +5% crit chance per level — crits multiply ore yield
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'critChance')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.mining?.critChance)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('mining', 'critChance')
                      ] || 0) < getUpgradeCost(state.tools?.mining?.critChance)
                    "
                    class="bg-yellow-600 hover:bg-yellow-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                    Upgrade
                ...
                >
                  <div>
                    <h4 class="font-bold text-orange-300/80">
                      🔥 Deep Strike Power (Lvl
                      {{ state.tools?.mining?.critDamage || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Crit yield ×{{
                        (state.tools?.mining?.critDamage || 1) + 1
                      }}
                      — grows with each level
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('mining', 'critDamage')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.mining?.critDamage)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    ...
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('mining', 'critDamage')
                      ] || 0) < getUpgradeCost(state.tools?.mining?.critDamage)
                    "
                    class="bg-orange-600 hover:bg-orange-500 disabled:bg-slate-700 disabled:text-slate-500 text-white py-1.5 px-4 rounded font-medium transition-colors"
                  >
                    Upgrade
            ...
            >
              ...
              <div class="flex flex-col gap-4">
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Lightweight Handle (Lvl
                      {{ state.tools?.foraging?.handle || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Increases gathering speed by 25%
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'handle')
                        ] || 0) >= getUpgradeCost(state.tools?.foraging?.handle)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.foraging?.handle) }}
                      {{ getUpgradeResource("foraging", "handle") }}
                    </p>
                  </div>
                  <button
                    @click="upgradeTool('foraging', 'handle')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'handle')
                      ] || 0) < getUpgradeCost(state.tools?.foraging?.handle)
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Curved Blade (Lvl {{ state.tools?.foraging?.metal || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Increases fiber yield by +1
                    </p>
                    <p
                      class="text-xs mt-1"
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'metal')
                        ] || 0) >= getUpgradeCost(state.tools?.foraging?.metal)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.foraging?.metal) }}
                      {{ getUpgradeResource("foraging", "metal") }}
                    </p>
                  </div>
                  <button
                    @click="upgradeTool('foraging', 'metal')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'metal')
                      ] || 0) < getUpgradeCost(state.tools?.foraging?.metal)
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Comfort Grip (Lvl {{ state.tools?.foraging?.grip || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      5% chance per level to double yield
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'grip')
                        ] || 0) >= getUpgradeCost(state.tools?.foraging?.grip)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                      Cost: {{ getUpgradeCost(state.tools?.foraging?.grip) }}
                      {{ getUpgradeResource("foraging", "grip") }}
                    </p>
                  </div>
                  <button
                    @click="upgradeTool('foraging', 'grip')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'grip')
                      ] || 0) < getUpgradeCost(state.tools?.foraging?.grip)
                ...
                >
                  <div>
                    <h4 class="font-bold text-slate-200">
                      Growth Enchantment (Lvl
                      {{ state.tools?.foraging?.enchantment || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      5% chance per level to double XP
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'enchantment')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.foraging?.enchantment)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    @click="upgradeTool('foraging', 'enchantment')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'enchantment')
                      ] || 0) <
                ...
                >
                  <div>
                    <h4 class="font-bold text-yellow-300/80">
                      ⚡ Keen Edge (Lvl
                      {{ state.tools?.foraging?.critChance || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      +5% crit chance per level — crits multiply yield
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'critChance')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.foraging?.critChance)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    @click="upgradeTool('foraging', 'critChance')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'critChance')
                      ] || 0) <
                ...
                >
                  <div>
                    <h4 class="font-bold text-orange-300/80">
                      🔥 Master Harvester (Lvl
                      {{ state.tools?.foraging?.critDamage || 1 }})
                    </h4>
                    <p class="text-xs text-slate-400">
                      Crit yield ×{{
                        (state.tools?.foraging?.critDamage || 1) + 1
                      }}
                      — grows with each level
                    </p>
                    <p
                      ...
                      :class="
                        (state.inventory[
                          getUpgradeResource('foraging', 'critDamage')
                        ] || 0) >=
                        getUpgradeCost(state.tools?.foraging?.critDamage)
                          ? 'text-green-400'
                          : 'text-red-400'
                      "
                    >
                  ...
                  <button
                    @click="upgradeTool('foraging', 'critDamage')"
                    :disabled="
                      (state.inventory[
                        getUpgradeResource('foraging', 'critDamage')
                      ] || 0) <
      ...
      <div class="w-full md:w-80 flex flex-col gap-6">
        ...
        <div
          ...
          class="bg-amber-900/20 border border-amber-500/30 p-4 rounded-xl shadow-lg"
        >
          ...
          <p
            v-if="nextWorkerUnlockLevel()"
            class="text-xs text-amber-300/50 mt-2"
          >
            Next worker unlocks at level {{ nextWorkerUnlockLevel() }}
          </p>
          <p v-else class="text-xs text-amber-300/50 mt-2">
            Max workers reached!
          </p>
        </div>
        ...
        <div
          v-if="state.workersTotal === 0"
          class="bg-slate-800 border border-slate-700 p-4 rounded-xl shadow-lg"
        >
        ...
        <div
          class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-xl flex-1 flex flex-col"
        >
          <h2
            class="text-lg font-bold text-white mb-4 flex items-center gap-2 pb-3 border-b border-slate-800"
          >
          ...
          <div class="space-y-2 flex-1">
            <template v-for="item in ALL_MATERIALS" :key="item.name">
              ...
              <div
                v-if="state.level >= item.unlockLevel"
                class="flex justify-between items-center bg-slate-950 p-3 rounded-lg border border-slate-800"
              >
                <div class="flex items-center gap-3">
                  <Axe
                    v-if="item.name.includes('Log')"
                    :size="16"
                    class="text-slate-500"
                  />
                  <Gem
                    v-else-if="item.name.includes('Ore')"
                    :size="16"
                    class="text-slate-500"
                  />
                ...
                <span
                  class="font-bold text-white font-mono bg-slate-800 px-2 py-1 rounded"
                >
      ...
      <button
        @click="fetchLeaderboard('level')"
        class="bg-yellow-600 p-2 rounded text-white font-bold mb-4"
      >
        🏆 View High Scores
      </button>

      <div
        v-if="leaderboard.isOpen"
        class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4"
      >
        <div
          class="bg-slate-900 border border-slate-700 rounded-xl max-w-lg w-full overflow-hidden"
        >
          <div
            class="p-4 bg-slate-950 flex justify-between items-center border-b border-slate-700"
          >
            <h2 class="text-xl font-bold text-white">Leaderboards</h2>
            <button
              @click="leaderboard.isOpen = false"
          ...
          <div class="flex border-b border-slate-700 bg-slate-900">
            <button
              ...
              :class="{
                'bg-slate-800 text-yellow-400':
                  leaderboard.activeTab === 'level',
              }"
              class="flex-1 p-3 text-slate-300 font-bold hover:bg-slate-800 transition"
            >
              Highest Level
            </button>
            <button
              @click="fetchLeaderboard('total_clicks')"
              :class="{
                'bg-slate-800 text-blue-400':
                  leaderboard.activeTab === 'total_clicks',
              }"
              class="flex-1 p-3 text-slate-300 font-bold hover:bg-slate-800 transition"
            >
              Most Clicks
            ...
            <button
              @click="fetchLeaderboard('total_gathered')"
              :class="{
                'bg-slate-800 text-green-400':
                  leaderboard.activeTab === 'total_gathered',
              }"
              class="flex-1 p-3 text-slate-300 font-bold hover:bg-slate-800 transition"
            >
              Most Gathered
          ...
          <div class="p-4 space-y-2 max-h-[60vh] overflow-y-auto">
            ...
            >
              ...
              <div class="font-mono text-lg font-bold">
                <span
                  v-if="leaderboard.activeTab === 'level'"
                  class="text-yellow-400"
                  >XP: {{ player.xp }}</span
                >
                <span
                  v-else-if="leaderboard.activeTab === 'total_clicks'"
                  class="text-blue-400"
                  >{{ player.total_clicks || 0 }} Clicks</span
                >
                <span
                  v-else-if="leaderboard.activeTab === 'total_gathered'"
                  class="text-green-400"
                  >{{ player.total_gathered || 0 }} Items</span
                >
              </div>
            </div>
